...
export async function load({ locals, url }) {
	...
	try {
		...
		const stats = await dbHelpers.getAttendanceStats(user.id, startDate, endDate);
		...
		return {
			...
			monthName: new Intl.DateTimeFormat('id-ID', { month: 'long' }).format(new Date(parseInt(year), parseInt(month) - 1))
		};
	} catch (error) {
		...
		return {
			user,
			attendanceRecords: [],
			stats: { total: 0, hadir: 0, terlambat: 0, tidak_hadir: 0, izin: 0, sakit: 0 },
			currentMonth: parseInt(month),
			currentYear: parseInt(year),
			monthName: new Intl.DateTimeFormat('id-ID', { month: 'long' }).format(new Date(parseInt(year), parseInt(month) - 1))
		};
