...
export const actions = {
	updateSchool: async ({ request, locals }) => {
		...
		try {
			...
			const schoolData = {
				...
				school_principal_name: formData.get('school_principal_name')?.toString() || '',
				school_principal_nip: formData.get('school_principal_nip')?.toString() || ''
			};
	...
	importDatabase: async ({ request, locals }) => {
		...
		try {
			...
			return fail(500, { 
				message: 'Gagal melakukan import database. Pastikan file valid dan coba lagi.' 
			});
		}
	}
};