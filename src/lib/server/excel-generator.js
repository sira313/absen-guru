...
export async function generateTPPReport(month, year, workDays, employeeTypeFilter = null) {
	try {
		...
		// Get school principal data from users with position 'Kepala Sekolah'
		const principalResult = await client.execute({
			sql: `SELECT name, nip FROM users WHERE position = 'Kepala Sekolah' AND role = 'guru' LIMIT 1`,
			args: []
		});
		...
		worksheet.getCell('C3').value = `: ${schoolData.school_name || 'Nama Sekolah'}`;
		...
		const principalName = principalData?.name || schoolData.school_principal_name || 'Nama Kepala Sekolah';
		const principalNip = principalData?.nip || schoolData.school_principal_nip || 'NIP Kepala Sekolah';
...
export async function generateMonthlyReport(month, year) {
	try {
		...
		for (let day = 1; day <= daysInMonth; day++) {
			...
			worksheet.mergeCells(worksheet.getCell(5, colIndex).address + ':' + worksheet.getCell(6, colIndex).address);
			headerRow1.getCell(colIndex).value = day;
		}

		// Right side header
		const ketCol = 6 + daysInMonth;
		worksheet.mergeCells(worksheet.getCell(5, ketCol).address + ':' + worksheet.getCell(6, ketCol).address);
		headerRow1.getCell(ketCol).value = 'JUMLAH';

		const sakitCol = ketCol + 1;
		worksheet.mergeCells(worksheet.getCell(5, sakitCol).address + ':' + worksheet.getCell(6, sakitCol).address);
		headerRow1.getCell(sakitCol).value = 'SAKIT';

		const izinCol = sakitCol + 1;  
		worksheet.mergeCells(worksheet.getCell(5, izinCol).address + ':' + worksheet.getCell(6, izinCol).address);
		headerRow1.getCell(izinCol).value = 'IZIN';

		const ketTotalCol = izinCol + 1;
		worksheet.mergeCells(worksheet.getCell(5, ketTotalCol).address + ':' + worksheet.getCell(6, ketTotalCol).address);
		...
		users.forEach((user, index) => {
			...
			let hadirCount = 0, sakitCount = 0, izinCount = 0;
			...
			for (let day = 1; day <= daysInMonth; day++) {
				...
				cell.value = code;
				cell.font = { name: 'Arial', size: 8, color: { argb: 'FF' + fontColor } };
				cell.alignment = { horizontal: 'center', vertical: 'middle' };
			}
		...
		worksheet.getCell(signatureRow, signatureCol).value = `PERJI, ${monthNames[month]} ${year}`;
		worksheet.getCell(signatureRow + 2, signatureCol).value = 'KEPALA SEKOLAH';
		worksheet.getCell(signatureRow + 3, signatureCol).value = schoolData.school_name || 'NAMA SEKOLAH';
		...
		worksheet.getCell(signatureRow + 7, signatureCol).value = `NIP. ${principalNip}`;
