...
export function getDayTypeInfo(date) {
  ...
  if (dayOfWeek === 0 || dayOfWeek === 6) {
    ...
    rules =
      "07:00-07:30: Upacara | 07:31-08:00: Hadir | 08:01-10:00: Terlambat";
  } else {
    dayType = "regular_day";
...
export function calculateAttendanceStats(attendanceRecords) {
  ...
  stats.attendanceRate =
    stats.totalDays > 0
      ? (((stats.hadir + stats.dinas_luar) / stats.totalDays) * 100).toFixed(1)
      : 0;

  stats.ceremony.participationRate =
    stats.ceremony.totalMondays > 0
      ? (
          (stats.ceremony.ikut_upacara / stats.ceremony.totalMondays) *
          100
        ).toFixed(1)
      : 0;
...
export function getAttendanceDescription(status, ceremonyStatus, date) {
  ...
  switch (status) {
    case "hadir":
      return "Hadir tepat waktu (07:00-07:30)";
    case "terlambat":
      return "Terlambat (07:31-10:00)";
    case "sakit":
      return "Sakit dengan surat keterangan";
    case "izin":
      return "Izin dengan surat keterangan";
    case "dinas_luar":
      return "Dinas luar kantor";
    case "tidak_hadir":
